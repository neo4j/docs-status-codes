...
|Title
a|The query returns no results due to repeated references to a relationship.
|Severity
m|WARNING
|Category
m|GENERIC
...
.Repeating a relationship variable
====
Query::
+
[source,cypher]
...
Suggestions for improvement::
In order to find all relationships which have a start node with label `A` and end node with label `B`, it could be rewritten to the following:
+
[source, cypher, role="noplay"]
----
