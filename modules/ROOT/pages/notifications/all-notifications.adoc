...
If you want to use that variable instead, it must be imported into the subquery using importing WITH clause. (the shadowing variable is: v)
|Category
m|GENERIC
|GQLSTATUS code
m|03N60
...
The variable `v` in the subquery uses the same name as a variable from the outer query.
Use `WITH v` in the subquery to import the one from the outer scope unless you want it to be a new variable.
|Classification
m|GENERIC
|SeverityLevel
m|INFORMATION
...
The query plan cannot be cached and is not executable without `EXPLAIN` due to the undefined parameter(s) `$param1, $param2`.
