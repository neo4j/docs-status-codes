...
Suggestions for improvement::
Extract the subquery expression into a preceding `WITH` and/or wrap the mutating statement into a `CALL` subquery.
For further explanation please contact support.
+
[source,cypher]
----
WITH COUNT {()} AS i
