...
* <<_performance_notifications, `PERFORMANCE` notification codes>>
* <<_hint_notifications, `HINT` notification codes>>
* <<_unrecognized_notifications, `UNRECOGNIZED` notification codess>>
* <<_unsupported_notifications, `UNSUPPORTED` notification codes>>
* <<_deprecated_notifications, `DEPRECATION` notification codes>>
* <<_generic, `GENERIC` notifications>>


[#_performance_notifications]
== `PERFORMANCE` notification codes
...
Multiple disconnected patterns in a query part build a cartesian product between all those parts.
This may produce a large amount of data and slow down query processing.
While occasionally intended, it may often be possible to reformulate the query to avoid the use of this cross-product, for example, by adding a relationship between the different parts or by using `OPTIONAL MATCH (identifier is: (p))`.
...
|Title
a|The provided pattern is unbounded.
Consider adding an upper limit to the number of node hops.
|Severity
m|INFORMATION
|Category
m|PERFORMANCE
...
Description of the returned code::
Using the shortest path with an unbounded pattern will likely result in long execution times.
...
|Title
a|Exhaustive shortest path has been planned for your query, which means that the shortest path graph algorithm might not be used to find the shortest path.
Hence, an exhaustive enumeration of all paths might be used to find the requested shortest path.
|Severity
m|INFORMATION
|Category
m|PERFORMANCE
...
Description of the returned code::
Using the shortest path with an exhaustive search fallback might cause the query to slow down since shortest path graph algorithms might not work for this use case.
It is recommended to use `WITH` to separate the `MATCH` containing the shortest path from the existential predicates on that path.
...
Description of the returned code::
Using `LOAD CSV` followed by a `MATCH` or `MERGE` clause that matches a non-indexed label will most likely not perform well on large data sets.
...
|Title
a|The execution plan for this query contains an eager operator, which forces all dependent data to be materialized in the main memory before proceeding.
|Severity
m|INFORMATION
|Category
m|PERFORMANCE
...
Description of the returned code::
Using LOAD CSV with a large data set in a query where the execution plan contains an eager operator could potentially consume a lot of memory and is likely to not perform well.
...
Description of the returned code::
Using a dynamic property makes it impossible to use an index lookup for this query (the indexed label is `Person`).
...
Description of the returned code::
Using a dynamic property makes it impossible to use an index lookup for this query (the indexed relationship type is `KNOWS`).
...
Description of the returned code::
One of the relationship types in your query is not available in the database, make sure you did not misspell it or that the label is available when you run this statement in your application (the missing relationship type is: NonExistingType)
...
The parallel runtime is experimental and might suffer from instability and potential correctness issues.
...
Description of the returned code::
The semantics of using a colon in the separation of alternative relationship types will change in a future version.
...
Use a path with a length of `1 [r*1..1]` instead or `MATCH` with a limit.
...
Description of the returned code::
Databases and aliases with unescaped `.` are deprecated unless indicated that they belong to a composite database. Names containing `.` should be escaped. 
(The database name is `foo.bar`.)
...
Description of the returned code::
The query uses a deprecated procedure. 
(`unsupported.dbms.shutdown` is no longer supported.)
...
Description of the returned code::
The query uses a deprecated runtime option.
(`runtime=interpreted` is deprecated, use `runtime=slotted` instead.)
...
Description of the returned code::
The `text-1.0` provider for text indexes is deprecated.
Use `text-2.0` instead.
...
|Title
a|Variable in a subquery is shadowing a variable with the same name from the outer scope.
|Severity
m|INFORMATION
|Category
m|GENERIC
...
Description of the returned code::
A variable in a subquery is shadowing a variable with the same name from the outer scope.
If you want to use that variable instead, it must be imported into the subquery using the importing `WITH` clause. (The shadowing variable is `n`.)
...
Description of the returned code::
You need to supply the query with parameters.
The produced query plan cannot be cached and is not executable without `EXPLAIN`.
(The missing parameters are `param`.)
