...
Notifications in the security category indicates that the result of the query or command might have a potential security issue.
Make sure that the behaviour is what was intended.
...
.Granting a role to a user who already have that role
====
Command::
+
[source, cypher]
...
`GRANT ROLE admin TO neo4j` has no effect.

Description of the returned code::
The user already has the role. See Status Codes documentation for more information.

Suggestions for improvement::
Make sure this was the intended role and user.
====
...
[source, cypher]
----
REVOKE ROLE admin, reader FROM neo4j
----

Full title of the returned code::
`REVOKE ROLE reader FROM neo4j` has no effect.
...
.Granting or denying a privilege to a role that already have that privilege
====
...
Suggestions for improvement::
Make sure this was the intended privilege and role.
====
...
[source, cypher]
----
REVOKE WRITE ON GRAPH * FROM editor
----

Full title of the returned code::
`REVOKE DENY WRITE ON GRAPH * FROM editor` has no effect.
...
[source, cypher]
----
REVOKE ROLE Foo, reader FROM neo4j
----

Full title of the returned code::
`REVOKE ROLE Foo FROM neo4j` has no effect.
...
Suggestions for improvement::
Make sure nothing is misspelled and that this was the intended role and user.
====
...
[source, cypher]
----
REVOKE ROLE reader FROM Foo
----

Full title of the returned code::
`REVOKE ROLE reader FROM Foo` has no effect.
...
[source, cypher]
----
REVOKE GRANT WRITE ON GRAPH * FROM Foo
----

Full title of the returned code::
`REVOKE GRANT WRITE ON GRAPH * FROM Foo` has no effect.
...
Suggestions for improvement::
Make sure nothing is misspelled and that this was the intended role.
====

.Revoking a privilege on a non-existing graph/database from a role
====
Command::
+
[source, cypher]
----
REVOKE GRANT WRITE ON GRAPH Foo FROM editor
----

Full title of the returned code::
`REVOKE GRANT WRITE ON GRAPH Foo FROM editor` has no effect.

Description of the returned code::
Database 'Foo' does not exist. Make sure nothing is misspelled.
This notification will become an error in a future major version.
See Status Codes documentation for more information.

Suggestions for improvement::
Make sure nothing is misspelled and that this was the intended graph/database.
====

.Revoking a privilege from a role with wildcard graph/database parameter
====
Parameter::
+
[source, javascript]
...
Description of the returned code::
Parameterized database and graph names do not support wildcards. Make sure nothing is misspelled.
This notification will become an error in a future major version.
See Status Codes documentation for more information.
