...
|Title
a|Vector index created without configured dimensions.
|Description
a|The vector index was created without `vector.dimensions`. 
This is allowed, but providing dimensions when creating a vector index is recommended, as it ensures that only vectors of that size are indexed and makes dimension mismatches fail clearly at query time. 
...
The vector index was created without `vector.dimensions`. 
This is allowed, but providing dimensions when creating a vector index is recommended, as it ensures that only vectors of that size are indexed and makes dimension mismatches fail clearly at query time. 
For example, set `OPTIONS { indexConfig: { ``vector.dimensions``: 1536 } }` when creating the index.
|SeverityLevel
m|INFORMATION
