...
|Title
a|The query execution plan contains a post-processing step that requires the local engine to fully materialize intermediate results. This may transfer large amounts of data from the remote source and increase memory usage. Consider rewriting the query so that aggregation, sorting, distinct or limit can be pushed down to the remote source.
|Description
|The query execution plan contains a post-processing step that requires the local engine to fully materialize intermediate results. This may transfer large amounts of data from the remote source and increase memory usage. Consider rewriting the query so that aggregation, sorting, distinct or limit can be pushed down to the remote source.
|Category
m|PERFORMANCE
...
m|03N97
|Status description
a|info: virtual graph post-processing.
The query execution plan contains a post-processing step that requires the local engine to fully materialize intermediate results.
This may transfer large amounts of data from the remote source and increase memory usage.
Consider rewriting the query so that aggregation, sorting, distinct or limit can be pushed down to the remote source.
|Classification
m|PERFORMANCE
|SeverityLevel
m|INFORMATION
