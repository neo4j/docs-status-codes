...
|Description
a|The virtual graph query execution plan contains a post-processing step that requires the local engine to fully materialize intermediate results. This may transfer large amounts of data from the remote source, increasing memory usage. Consider rewriting the query so that aggregation, `ORDER BY`, `DISTINCT`, and `LIMIT` can be pushed down to the remote source.
|Category
m|PERFORMANCE
|GQLSTATUS code
m|03N97
|Status description
a|warn: virtual graph post-processing. The virtual graph query execution plan contains a post-processing step that requires the local engine to fully materialize intermediate results. This may transfer large amounts of data from the remote source, increasing memory usage. Consider rewriting the query so that aggregation, `ORDER BY`, `DISTINCT`, and `LIMIT` can be pushed down to the remote source.
|Classification
m|PERFORMANCE
|SeverityLevel
m|WARNING
