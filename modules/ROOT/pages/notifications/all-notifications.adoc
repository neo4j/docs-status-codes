...
An index exists on label/type(s) `$label/$type`.
...
m|01N40
|StatusDescription
a|info: runtime unsupported.
The query cannot be executed `runtime=pipelined operatorEngine=compiled expressionEngine=compiled`, `runtime=pipelined operatorEngine=interpreted expressionEngine=compiled` is used. Cause: `method too big`.
|Severity
m|INFORMATION
|Classification
m|PERFORMANCE
