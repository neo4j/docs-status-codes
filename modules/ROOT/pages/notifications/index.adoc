...
* *Code* -- The status code in the form of `Neo.ClientNotification.[SubType].[Name]`.
...
| Make sure that the behavior is what you intended.
...
| PERFORMANCE
| INFORMATION
| The query uses costly operations and might be slow. Consider if it is possible to rewrite the query differently.
|
|===