...
Status description:: error: syntax error or access rule violation - index or constraint value too long. The provided index or constraint `{ <<item>> }` `{ <<input>> }` (`{ <<bytes>> }` bytes) exceeded limit of `{ <<bytes>> }` bytes.
