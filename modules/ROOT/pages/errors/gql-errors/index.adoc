...
Status description:: error: syntax error or access rule violation - procedure return column not defined. The procedure return column `{ <<variable>> }` is not defined for this procedure. Available output columns are: `{ <<variableList>> }`. Verify that the spelling is correct.
