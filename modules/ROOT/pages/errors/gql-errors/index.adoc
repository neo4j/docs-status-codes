...
Status description:: error: system configuration or operation exception - secret provider not enabled. Secret provider `{ <<secretProviderType>> }` is not available. Enable it with setting `{ <<cfgSetting>> }`.
