...
Try querying it with a vector with a dimension other than 1048:
...
The query returns an error with GQLSTATUS 51N65 and a status description:

.GQLSTATUS error
[source, error]
----
51N65: system configuration or operation exception - vector index dimensionality mismatch. Vector index `indexName` has a configured dimensionality of 1048, but the provided vector has a dimension 3.
----
