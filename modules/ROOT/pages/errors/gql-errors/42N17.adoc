...
[NOTE]
====
This example is valid only for versions prior to Neo4j 2026.06.
From 2026.06 onwards, the `WHERE` clause, when part of procedure calls, is allowed on the `system` database, and the following example query works as expected.
====
...
[source, error]
----
42NA9: syntax error or access rule violation - system database procedure rules. The system database supports a restricted set of Cypher clauses. The supported clause structure for procedure calls is: 'CALL', 'YIELD', 'RETURN'. 'YIELD' and 'RETURN' clauses are optional. The order of the clauses is fixed and each can only occur once.
  42N17: syntax error or access rule violation - unsupported request. 'WHERE' is not allowed on the system database.
  42001: syntax error or access rule violation - invalid syntax
----
