...
Try using a property key other than `rating` in the `WHERE` clause property predicate in a `SEARCH` clause:
...
The query returns an error with GQLSTATUS 22ND3 and status description:

.GQLSTATUS error
[source, error]
----
22ND3: data exception - wrong property for vector search with filters. The property `votes` is not an additional property for vector search with filters on the vector index `moviePlots`.
----
