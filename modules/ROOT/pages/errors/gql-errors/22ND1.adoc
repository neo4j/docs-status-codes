...
== Status description
error: data exception - operation not allowed here. Invalid input: '{ <<query>> }' is not allowed for roles that are granted to an AUTH RULE.
...
[source]
----
error: data exception - operation not allowed here. Invalid input: 'DENY MATCH {*} ON GRAPH secret-db NODES * TO role' is not allowed for roles that are granted to an AUTH RULE.
----
