...
You also have two roles, called `role` and `otherrole`, and an auth rule, called `authrule`, where `role` is granted to `authrule` and `otherrole` is not.

Try denying privileges to the role:
...
For example, you can replace the generic deny from step 1 in the <<example-scenario-22nd1, Example scenario>> with the following more fine-grained alternative:
