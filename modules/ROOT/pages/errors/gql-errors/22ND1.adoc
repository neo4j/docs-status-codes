...
The example assumes that you have a database called `secretDb`, two roles, called `role` and `otherrole`, and an auth rule, called `authrule`, where `role` is granted to `authrule` and `otherrole` is not.
...
For example, you can replace the generic deny from the <<example-scenario-22nd1, Example scenario>> with the following more fine-grained alternative by granting privileges to the role for specific graphs instead of denying access to all nodes in the `secretDb` graph:
