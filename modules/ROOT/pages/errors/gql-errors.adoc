...
Status description:: error: data exception - invalid shortest path expression. Shortest path expressions must contain start and end nodes. Cannot find: { $var }.
...
Status description:: error: data exception - merge node uniqueness constraint violation. The 'MERGE' clause did not find a matching node { $var } and cannot create a new node due to conflicts with existing uniqueness constraints. 

=== 22N42

Status description:: error: data exception - merge relationship uniqueness constraint violation. The 'MERGE' clause did not find a matching relationship { $var } and cannot create a new relationship due to conflicts with existing uniqueness constraints.
...
// Status description:: error: data exception - feature not supported on Neo4j Community Edition. The { $feat } feature is not supported by Neo4j Community Edition.
...
Status description:: error: data exception - parsing JSON exception. Underlying error: { $cause }.
