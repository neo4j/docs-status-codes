...
* Do not use error messages from underlying libraries/dependencies (e.g. Java runtime errors) but always catch the error and throw your own error message.

* For wrapped errors, it is safe to reuse error messages only if they are coming from Neo4j, i.e. have a status.
