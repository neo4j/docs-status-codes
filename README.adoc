...
| See <<example, Example>>.
2+| Contains one or more example queries to illustrate the possible scenarios when this notification or error would be returned.
...
If you are adding a new error, please add the example section as well but comment it out.
When we update the Status Codes documentation to 6.0, we will uncomment all examples for errors.
====
